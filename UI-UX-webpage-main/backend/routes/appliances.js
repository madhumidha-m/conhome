...
router.get('/:roomId', auth, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM appliances WHERE room_id=$1 AND user_id=$2',
...
router.patch('/:id/toggle', auth, async (req, res) => {
  try {
    ...
    const check = await pool.query(
      'SELECT appliance_id FROM appliances WHERE appliance_id=$1 AND user_id=$2',
    ...
    await pool.query(
      'INSERT INTO appliance_status (status_id, appliance_id, power_state, updated_at) VALUES (gen_random_uuid(), $1, $2, NOW())',
      [req.params.id, power_state]
    )
    await pool.query(
      'UPDATE appliances SET power=$1 WHERE appliance_id=$2',
...
router.patch('/:id', auth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE appliances SET appliance_name=$1 WHERE appliance_id=$2 RETURNING *',
      [appliance_name, req.params.id]
    )
    res.json(result.rows[0])
  } catch (err) {
    console.error('RENAME appliance error:', err)
    res.status(500).json({ error: err.message })
  }
})
