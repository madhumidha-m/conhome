...
export default function Stats() {
  ...
  useEffect(() => {
    ...
    fetch('/api/current', {
      ...
      .then(data => {
        ...
        data.forEach(d => {
          const month = monthNames[new Date(d.created_at).getMonth()]
          monthMap[month] += parseFloat(d.energy_kwh || 0)
        })
        const graphData = monthNames.map(month => ({
          time: month,
          power: parseFloat(monthMap[month].toFixed(4))
        }))
        setCurrentData(graphData)

        const thisMonth = monthNames[new Date().getMonth()]
        setMonthlyKwh(parseFloat(monthMap[thisMonth].toFixed(2)))
      })
  ...
  const statCards = [
    ...
    { label: 'Monthly kWh',   value: monthlyKwh,    icon: Zap,      color: '#ff4d4f' },
  ]
    ...
    <div>
      ...
      {showSetupModal && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',backdropFilter:'blur(6px)',zIndex:100,display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{background:'white',borderRadius:24,padding:30,width:360,textAlign:'left'}}>
            <h2 style={{color:'#3b4140'}}>Add ESP32 Device</h2>
            <p><b>Step 1:</b> Power ON the ESP32.</p>
            <p><b>Step 2:</b> Connect to WiFi:<br /><b>ESP32_SETUP</b></p>
            <p>Password:<b> setup1234</b></p>
            <p><b>Step 3:</b> Open setup page and enter your WiFi credentials.</p>
            <button
              onClick={() => window.open('http://192.168.4.1', '_blank')}
              style={{width:'100%',padding:12,marginTop:10,border:'none',borderRadius:10,background:'#847f7c',color:'white',fontWeight:'bold',cursor:'pointer'}}
            >
              Open Setup Page
            </button>
            <button
              onClick={async () => {
                try {
                  const res = await fetch('/api/enroll', {
                    ...
                    body: JSON.stringify({ device_id: scannedDeviceId })
                  })
                  const data = await res.json()
                  if (data.success) {
              ...
              style={{width:'100%',padding:12,marginTop:10,borderRadius:10,border:'none',background:'#4caf88',color:'white',fontWeight:'bold',cursor:'pointer'}}
            >
              I've Entered Credentials
            </button>
            <button
              onClick={() => setShowSetupModal(false)}
              style={{width:'100%',padding:12,marginTop:10,borderRadius:10,border:'none',background:'#f0f0f0',color:'#666',cursor:'pointer'}}
            >
              Close
            </button>
          </div>
      ...
      {showSuccess && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:999,display:'flex',justifyContent:'center',alignItems:'center'}}>
          <div style={{background:'white',padding:'25px',borderRadius:'20px',width:'300px',textAlign:'center'}}>
            <h2>Success ✅</h2>
            <p>Device Enrolled Successfully</p>
            <button
              onClick={() => setShowSuccess(false)}
              style={{padding:'10px 20px',borderRadius:'10px',border:'none',background:'#847f7c',color:'white'}}
            >
              OK
            </button>
          </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',backdropFilter:'blur(6px)',zIndex:100,display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{background:'white',borderRadius:24,padding:36,width:360,textAlign:'center',boxShadow:'0 20px 60px rgba(0,0,0,0.25)'}}>
            ...
            <div style={{width:'100%',margin:'0 auto 20px',borderRadius:16,overflow:'hidden',border:'3px solid #9ca3af'}}>
              <video ref={videoRef} style={{width:'100%',height:300,objectFit:'cover',display:'block'}} autoPlay playsInline muted />
        ...
        <div className={`${styles.chartWide} glass`}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
            ...
            <button
              ...
              style={{padding:'6px 14px',borderRadius:20,border:'1.5px solid rgba(66,64,64,0.3)',background:showMore?'#2d6a4f':'transparent',color:showMore?'white':'#9ca3af',fontSize:12,fontWeight:700,cursor:'pointer',transition:'all 0.2s'}}
            >
              More →
            </button>
          </div>
